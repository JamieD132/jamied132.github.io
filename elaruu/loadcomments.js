...
if(sessionStorage.getItem('session')){
    ...
    if(session.username && session.password){
        fetch('https://api.jamied132.workers.dev/auth/signin',{method:'POST',headers:JSON.stringify({Authorization:btoa(JSON.stringify(session))})}).then(res=>res.json()).then(j=>{
            if(!j.error){
                signedin=true;
